...
| 1 | **Opus** | ✅ **O1 — Modelo de datos + algoritmo de progresión.** Cerrar esquemas de `plan.json`, del log de sesión y de `hitos.json` (logros/hitos/premios con estado activo/inactivo y condición de desbloqueo); especificar las reglas subir/mantener/bajar (RIR vs objetivo, dolor 0-10, regla 24h, incrementos por ejercicio) como pseudocódigo testeable. | `docs/plan/schema.md`, `progresion.md`, `progresion.js`, `hitos.json`, tests |
| 2 | **Opus** | ✅ **O2 — Primer bloque de entreno (4-6 semanas).** Trabajo de *entrenador*: 3 sesiones/semana + día de casa, versión completa y mínima, RPE ≤7, accesorios justificados según lumbar/rodilla/hombro (ver `instrucciones.md`). En formato `bloque-actual.json`. | `docs/plan/bloque-actual.json` + `.md` |
